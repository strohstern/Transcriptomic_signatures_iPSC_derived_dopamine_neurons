...
subtitle: Additional Figure preparation for paper
output:
  html_document:
    df_print: paged
---
...
```{r}
RMD_file_name <- "5-1.Adendum_figures"
```
...
TH_SNc_markers <-
  read_tsv(
    here(
      "input_data",
      "marker_genes_human_PD_brain_LCM",
      "Marker_genes_TH_SNc_intersect_top5percent_TPM.tsv"
    )
  )
...
ggsave(
  ...
  filename = str_c(
    figures_out,
    "UMAP_plot_all_cells_cluster_id_detailed.pdf",
    sep = "/"
  ),
  device = "pdf", width = 11, height = 7
)
...
pdf(
  file = str_c(
    figures_out,
    "heatmap_marker_genes_detailed_cell_id_labels.pdf",
    sep = "/"
  )
)
hts
...
png(
  file = str_c(
    figures_out,
    "mDA_heatmap_marker_genes_detailed_cell_id_labels.png",
    sep = "/"
  )
)
hts$mDA
...
png(
  file = str_c(
    figures_out,
    "NPC_heatmap_marker_genes_detailed_cell_id_labels.png",
    sep = "/"
  )
)
hts$NPC
...
```{r}
tick_cols <- marker_genes %>% #arrange(symbol) %>% 
{structure(.$anno_colour, names = .$symbol)}

# tick_cols <- c(tick_cols, "SNCA" = "#000000")
```
...
png(
  file = str_c(
    figures_out,
    "all_heatmap_marker_genes_detailed_cell_id_labels.png",
    sep = "/"
  )
)
hts_all
...
# Highly expressed genes in SNc TH neurons

We identified genes that are expressed in the top 5% of detected genes in SNc TH neuron datasets (project: RN19194, lab-Gandhi-Patani/inputs/babs/Sonia.gandhi/stephanie.strohbuecker/human_PD_brain_LCM/ Html report). 
For this a LCM isolated TH-stained neurons were isolated and analysed with a bulk RNA-seq approach. In addition, data from [Nichterwitz et al. 2016](https://pubmed.ncbi.nlm.nih.gov/27387371/) was used.

For each of the samples we identified the top 5% most highly expressed genes among the detected genes (i.e. TPM > 0). Subsequently we intersected the obtained gene lists to obtain the following genes:
```{r}
TH_SNc_markers %>% 
...
# hts_all <-
  map(seur_all, function(seur_obj) {
    ...
    mat <-
      GetAssayData(seur_obj, slot = "scale.data", assay = "RNA")
    TH_SNc_markers <- TH_SNc_markers %>% 
      filter(gene_name %in% rownames(mat))
    mat <- mat[TH_SNc_markers$gene_name,]
    dim(mat)
    col_anno <- seur_obj[[]] %>%
      as_tibble(rownames = "cell_id") %>%
      select(cell_id, cell_states.0.4) %>%
