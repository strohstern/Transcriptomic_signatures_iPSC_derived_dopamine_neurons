...
subtitle: Comparing Integration approaches
output:
  html_document:
    df_print: paged
---
...
We integrated across all samples with the "normal" pairwise approach.

```{r}
RMD_file_name <- "1-2.Clustering_of_integratated_dataset"
```
...
Seurat_obj_int <-
  readRDS(
    here(
      "output",
      "1-1.Integrate_samples_after_filtering_cluster",
...
```{r}
Seurat_obj_int <- list("integrated" = Seurat_obj_int)
```
...
Seurat_obj_int <- map(Seurat_obj_int, function(seur_obj){
...
Seurat_obj_int <- imap(Seurat_obj_int, function(seur_obj, seur_name) {
...
map(Seurat_obj_int, function(Seur_obj) {
...
map2(Seurat_obj_int, names(Seurat_obj_int), function(Seur_obj, Seur_obj_name) {
  DimHeatmap(Seur_obj, dims = 1:15, cells = 500, balanced = TRUE, fast = FALSE) %>% 
    annotate_figure(top = text_grob(Seur_obj_name,
                                            size = 32,
                                            face = "bold"))
})
```
...
map2(Seurat_obj_int, names(Seurat_obj_int), function(Seur_obj, Seur_obj_name) {
  ElbowPlot(Seur_obj, ndims = 50)  +
    ggtitle(Seur_obj_name)
}) %>% ggarrange(plotlist = ., nrow = 1, ncol = length(Seurat_obj_int))
```
...
```{r}
Seurat_obj_int$integrated@misc$nb_PCs <- 45
```
...
```{r}
Seurat_obj_int <-
  readRDS(str_c(output, "Seurat_obj_int_seurat_filtered_integrated_pre-processed.rds", sep = "/"))
```
...
map(names(Seurat_obj_int), function(Seur_name) {
  Seur_obj <- Seurat_obj_int[[Seur_name]]
  sub_text <- Seur_name
...
imap(marker_gene_list, function(gene_list, gene_list_name) {
  map(names(Seurat_obj_int), function(Seur_name) {
    ...
    ggarrange(
      ...
      legend = "bottom",
      nrow = 2,
      ncol = ceiling(length(Seurat_obj_int)/2),
      align = "hv"
    ) %>%
    annotate_figure(top = text_grob(
